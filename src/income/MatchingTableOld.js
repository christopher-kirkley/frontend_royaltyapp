...
function MatchingTable(props) {
	...
	return (
		<div>
		<Paper>
			<Typography variant="subtitle1">Set UPC</Typography>
			<Typography variant="subtitle1">Catalog == {catalog} </Typography>
		...
		<Container component={Paper}>
			...
			{ (rowsPerPage > 0 ?
				...
				<TableRow
				className={ isSelected(row.id) ? classes.selected : null }
				>
				{isSelected(row.id)}
						<input type="hidden"
							form={`form${row.id}`}
						...
						<TableCell
							...
							className={ distributorChecked && isSelected(row.id) ? classes.active : null }
						...
						<TableCell
							id={`upc${row.id}`}
							className={ upcChecked && isSelected(row.id) ? classes.active : null }
